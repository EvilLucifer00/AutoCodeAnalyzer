...
A lightweight Flask-based realtime source code analyzer that ingests a Git repository, builds an Google Gemini-backed embedding index with LangChain and Chroma, and answers developer questions from code.
...
- Query code with a conversational LLM chain via Gemini
...
```bash
conda create -n analyzer_pro python=3.11 -y
conda activate analyzer_pro
pip install -r requirements.txt
```
...
- This project uses Google Gemini embeddings and `langchain` for semantic search.
